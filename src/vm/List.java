...
public class List {
	...
	public HashMap<String, Integer> getpriceHM() {
		return price;
	}
	public static void readList() throws IOException {
		// 프로그램 실행 후 최초 1회만 실행한다.
		// 프로그램 실행 중에는 메모리에서 가격과 개수를 관리한다.
		BufferedReader reader = new BufferedReader(new FileReader("./src/stocklist.txt"));

		String str;
		while ((str = reader.readLine()) != null) {
			String[] tmp = str.split(" ");
			beverage.add(tmp[0]); 							//tmp[0] == 음료수이름
			count.put(tmp[0], Integer.parseInt(tmp[1]));	//음료수이름, 갯수
			price.put(tmp[0], Integer.parseInt(tmp[2]));    //음료수이름, 가격
		}
